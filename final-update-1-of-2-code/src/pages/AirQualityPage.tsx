...
import { aqiAdvice, aqiCat } from '../i18n/dynamic';
...
const AirQualityPage: React.FC = () => {
  ...
  if (status === 'ERROR' && !airQuality) {
    return (
      <Card>
        <ErrorState message={errorMessage ?? tr('Air quality data is currently unavailable.', 'वायु गुणवत्ता का डेटा अभी उपलब्ध नहीं है।')} onRetry={refresh} />
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-bold text-[var(--color-text-primary)]">{tr('Air Quality', 'वायु गुणवत्ता')}</h1>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        <Card className="lg:col-span-1 flex flex-col items-center justify-center">
          ...
          <p className="text-sm text-[var(--color-text-secondary)] text-center mt-3 leading-relaxed">
            {aqiAdvice(AQI_GUIDANCE[airQuality.current.aqiCategory], airQuality.current.aqiCategory)}
          </p>
        </Card>
      ...
      <Card>
        ...
        <div className="flex items-center gap-2 mt-2 text-sm text-[var(--color-text-muted)]">
          <Info size={12} /> {tr('Values after the "Now" marker are model forecast, not live sensor measurements.', 'चार्ट में "अभी" के निशान के बाद के मान मॉडल पूर्वानुमान हैं, लाइव सेंसर माप नहीं।')}
        </div>
      </Card>
          ...
          {SCALE.map((s) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-xs font-semibold text-[var(--color-text-primary)] leading-tight">{aqiCat(s.category)}</p>
      ...
      <p className="text-sm text-[var(--color-text-muted)] text-center">
        {tr('Source', 'स्रोत')}: {airQuality.meta.source} · {tr('Last updated', 'अंतिम अपडेट')} {lastUpdated ? new Date(lastUpdated).toLocaleTimeString() : '—'}
      </p>
    </div>
  );
};
