...
import { activityScoreRules, computeActivityScore, uvCategory } from '../../utils/activityScore';
...
import { tr, ph, cond, compass, aqiCat, aqiAdvice, severity } from '../../i18n/dynamic';
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-base text-[var(--color-text-secondary)] mt-0.5">
                {cond(weather.current.condition)} · <span className="font-semibold text-[var(--color-brand)]">{formatTemp(today.max, preferences.units)}</span>/{formatTemp(today.min, preferences.units)}
              </p>
            </div>
  ...
  return (
    <GlassCard tint={severe ? 'red' : 'blue'} onClick={() => navigate('/warnings')} className="!py-3.5">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-[15px] font-semibold text-[var(--color-text-primary)] line-clamp-1">{top.type} · {severity(top.severity)}</p>
...
export const ScoreCard: React.FC = () => {
  ...
  return (
    <GlassCard tint="green" className="flex flex-col items-center text-center">
      ...
      {open && <p className="text-xs text-[var(--color-text-secondary)] leading-relaxed mt-1 text-left">{activityScoreRules()}</p>}
    </GlassCard>
  );
};
...
export const WindCard: React.FC = () => {
  ...
  return <StatHalf tint="blue" icon={<Wind size={36} className="text-[var(--color-brand)]" />} title={t('m.card.wind')} value={String(kmh)} unit={preferences.units === 'imperial' ? tr('mph', 'मील/घं') : tr('km/h', 'किमी/घं')} sub={compass(c.windDirectionLabel)} subColor="var(--color-text-secondary)" />;
};
  ...
  return (
    <GlassCard onClick={() => navigate('/air-quality')}>
      ...
      <div className="flex items-end gap-3">
        ...
        <div className="pb-1">
          <p className="text-lg font-semibold" style={{ color }}>{a.aqi === null ? L('Unavailable', 'उपलब्ध नहीं') : aqiCat(a.aqiCategory)}</p>
      ...
      <p className="text-sm text-[var(--color-text-secondary)] mt-3 leading-snug">{aqiAdvice(AQI_GUIDANCE[a.aqiCategory], a.aqiCategory)}</p>
    </GlassCard>
  );
};
...
export const OutlookCard: React.FC = () => {
  ...
  return (
    <GlassCard tint="purple" onClick={() => navigate('/ai-predictions')}>
      ...
      />
      <p className="flex items-center gap-1.5 text-sm text-[var(--color-text-muted)] mb-1"><Trend size={15} /> {ph(insight.trend)}</p>
