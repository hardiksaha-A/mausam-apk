          ...
          <ul className="divide-y divide-[var(--color-panel-border)]">
            {(Object.keys(notifLabels) as (keyof NotificationPrefs)[]).map((k) => (
              <li key={k} className="flex items-center gap-3 py-3 first:pt-0">
                ...
                <Toggle on={preferences.notifications[k]} onChange={(v) => setNotificationPref(k, v)} label={notifLabels[k]} />
              </li>
            ))}
          </ul>
          <p className="text-xs text-[var(--color-warning)] mt-2 leading-snug">{t('m.profile.notifNote')}</p>
        </GlassCard>
      </section>
