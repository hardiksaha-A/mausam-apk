      ...
      <GlassCard>
        ...
        {(geoState === 'denied' || geoState === 'unavailable' || geoState === 'timeout' || geoState === 'error' || geoState === 'unsupported') && (
          <p className="mt-2 text-sm text-[var(--color-warning)] leading-snug">
            {geoState === 'denied' ? 'Location permission is off. Enable it in Android Settings → Apps → Mausam → Permissions, or search for a place above.' : 'Could not get your location right now. Try again, or search for a place above.'}
          </p>
        )}
      </GlassCard>
