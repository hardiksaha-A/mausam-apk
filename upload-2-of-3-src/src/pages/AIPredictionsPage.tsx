...
const AIPredictionsPage: React.FC = () => {
  ...
  if (status === 'ERROR' && !airQuality) {
    return (
      <Card>
        <ErrorState message={errorMessage ?? 'Prediction inputs unavailable.'} onRetry={refresh} />
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <h1 className="text-lg font-bold text-[var(--color-text-primary)]">AI Predictions</h1>
        <span className="inline-flex items-center gap-1.5 rounded-full border border-[var(--color-advanced)]/30 bg-[var(--color-advanced)]/10 px-2.5 py-1 text-sm font-medium uppercase tracking-wide text-[var(--color-advanced)]">
          <Sparkles size={11} /> Simulated AI Prediction
        </span>
      </div>
      ...
      <Card className="border-[var(--color-advanced)]/30 bg-[var(--color-advanced)]/[0.04]">
        <div className="flex gap-3">
          ...
          <p className="text-sm text-[var(--color-text-secondary)] leading-relaxed">
            Mausam does not yet run a trained machine learning model. The AQI/PM figures below for each horizon are
            drawn from Open-Meteo's real CAMS atmospheric forecast where available; the risk narrative, "confidence",
            and contributing-factor weighting are a prototype heuristic ({predictionService.modelLabel}) with no
            statistical validation. Treat this page as illustrative, not decision-grade forecasting.
          </p>
        </div>
      </Card>
      ...
      <Card hoverEffect={false}>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          <div className="flex items-center gap-3 shrink-0">
            ...
            <div>
              <p className="text-sm font-semibold text-[var(--color-text-primary)]">{explanation.trend}</p>
              <p className="text-sm text-[var(--color-text-muted)] flex items-center gap-1">
                <Gauge size={11} /> Prototype confidence: {explanation.confidenceLabel}
              </p>
            </div>
          </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3">
        {predictions.map((p) => (
          <Card key={p.horizonLabel} hoverEffect>
            <p className="text-sm font-medium text-[var(--color-text-muted)] uppercase tracking-wide mb-2">{p.horizonLabel}</p>
            <p className="text-2xl font-bold text-[var(--color-text-primary)] tabular-nums">{p.aqi}</p>
            <p className="text-sm text-[var(--color-text-muted)] mb-2">AQI · {classifyUsAqi(p.aqi)}</p>
            <span
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        ...
        <Card>
          ...
          <p className="text-sm text-[var(--color-text-muted)] mt-3">
            These percentages are a prototype estimate for illustration only — not scientifically verified source
            attribution.
          </p>
        </Card>
      </div>
