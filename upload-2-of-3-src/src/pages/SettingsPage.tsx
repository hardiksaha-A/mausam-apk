...
const SettingsPage: React.FC = () => {
  ...
  const [providerTests, setProviderTests] = useState<Record<ProviderKey, ProviderTestState>>({
    ...
    prediction: { status: 'ok', message: 'Prototype heuristic — always available, never fails.' },
  });
  ...
  const providers: { key: ProviderKey; name: string; kind: string; requiresKey: boolean; note: string }[] = [
    { key: 'weather', name: 'Open-Meteo Forecast API', kind: 'Weather', requiresKey: false, note: 'Free, keyless, CORS-enabled.' },
    { key: 'airQuality', name: 'Open-Meteo Air Quality API', kind: 'Air Quality', requiresKey: false, note: 'Free, keyless, CAMS atmospheric model.' },
    { key: 'geocoding', name: 'Open-Meteo Geocoding + BigDataCloud', kind: 'Geocoding', requiresKey: false, note: 'Forward search + reverse lookup, both keyless.' },
    { key: 'prediction', name: 'MockPredictionService (prototype)', kind: 'Prediction', requiresKey: false, note: 'No ML backend yet — see AI Predictions page.' },
  ];

  const notificationLabels: { key: keyof NotificationPrefs; label: string }[] = [
    { key: 'severeAqi', label: 'Severe AQI alerts' },
    { key: 'weatherAlerts', label: 'Weather alerts' },
    { key: 'pollutionSpikes', label: 'Pollution spikes' },
  ...
  return (
    <div className="space-y-5 max-w-3xl">
      ...
      <Card>
        ...
        <div className="mb-3">
          <p className="text-sm font-medium text-[var(--color-text-secondary)] mb-2">Saved places</p>
          {savedLocations.length > 0 ? (
            ...
            <p className="text-sm text-[var(--color-text-muted)] rounded-lg border border-dashed border-[var(--color-panel-border)] px-3 py-3">
              No saved places yet — tap the bookmark icon next to your location on the Dashboard to save one (Home, Work, etc).
            </p>
          )}
        </div>
      ...
      <Card>
        ...
        <p className="text-sm text-[var(--color-text-muted)]">
          Demo Mode uses a fixed, clearly-labeled sample dataset — never mixed with live provider responses.
        </p>
      </Card>
            ...
            return (
              <div key={p.key} className="rounded-lg border border-[var(--color-panel-border)] p-3">
                <div className="flex items-start justify-between gap-3 mb-1.5">
                  ...
                  >
                    {p.requiresKey ? 'Requires Key' : 'Configured'}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5 text-sm">
                    {test.status === 'ok' && <><CheckCircle2 size={13} className="text-[var(--color-accent)]" /> <span className="text-[var(--color-text-secondary)]">Connected{test.lastSuccess ? ` · ${new Date(test.lastSuccess).toLocaleTimeString()}` : ''}</span></>}
                    ...
                    {test.status === 'idle' && <span className="text-[var(--color-text-muted)]">Not tested yet</span>}
                    {test.status === 'testing' && <><Loader2 size={13} className="animate-spin text-[var(--color-text-muted)]" /> <span className="text-[var(--color-text-muted)]">Testing…</span></>}
                  </div>
                  <button
      ...
      <Card>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs">
          <div className="rounded-lg border border-[var(--color-panel-border)] p-2.5"><p className="text-[var(--color-text-muted)]">AQI High</p><p className="font-semibold text-[var(--color-text-primary)]">{ALERT_THRESHOLDS.aqiHigh}</p></div>
          <div className="rounded-lg border border-[var(--color-panel-border)] p-2.5"><p className="text-[var(--color-text-muted)]">AQI Critical</p><p className="font-semibold text-[var(--color-text-primary)]">{ALERT_THRESHOLDS.aqiCritical}</p></div>
          <div className="rounded-lg border border-[var(--color-panel-border)] p-2.5"><p className="text-[var(--color-text-muted)]">PM2.5 Spike</p><p className="font-semibold text-[var(--color-text-primary)]">{ALERT_THRESHOLDS.pm25Spike} µg/m³</p></div>
          <div className="rounded-lg border border-[var(--color-panel-border)] p-2.5"><p className="text-[var(--color-text-muted)]">Extreme Heat</p><p className="font-semibold text-[var(--color-text-primary)]">{ALERT_THRESHOLDS.extremeHeatC}°C</p></div>
          <div className="rounded-lg border border-[var(--color-panel-border)] p-2.5"><p className="text-[var(--color-text-muted)]">Heavy Rain</p><p className="font-semibold text-[var(--color-text-primary)]">{ALERT_THRESHOLDS.heavyRainProbability}%</p></div>
          <div className="rounded-lg border border-[var(--color-panel-border)] p-2.5"><p className="text-[var(--color-text-muted)]">Strong Wind</p><p className="font-semibold text-[var(--color-text-primary)]">{ALERT_THRESHOLDS.strongWindKph} km/h</p></div>
        </div>
      </Card>
      ...
      <Card>
        ...
        <p className="text-sm text-[var(--color-text-muted)] mt-3">
          {themePreference === 'system'
            ? `Following your device's setting — currently ${theme}.`
            : `Currently locked to ${theme} mode regardless of device setting.`}
        </p>
      </Card>
      ...
      <Card>
        ...
        <ul className="text-sm text-[var(--color-text-secondary)] space-y-2 leading-relaxed">
          <li><strong className="text-[var(--color-text-primary)]">AQI standard:</strong> US EPA Air Quality Index, as computed by the Open-Meteo Air Quality API.</li>
          <li><strong className="text-[var(--color-text-primary)]">Update behavior:</strong> Data refreshes on location change and manual refresh; there is no background polling yet.</li>
          <li><strong className="text-[var(--color-text-primary)]">Prediction status:</strong> {predictionService.modelLabel}. No trained ML model is connected.</li>
          <li><strong className="text-[var(--color-text-primary)]">Known limitation:</strong> Air-quality historical analytics beyond ~92 days are capped by the free provider and labeled Estimated.</li>
          <li><strong className="text-[var(--color-text-primary)]">Monitoring points:</strong> Pollution-map points are live modeled grid values, not physical ground sensors, unless Demo Mode is active.</li>
        </ul>
      </Card>
    </div>
  );
