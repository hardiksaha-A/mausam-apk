...
const GEO_ERROR_MESSAGES: Record<string, string> = {
  denied: 'Location permission denied. Enable it in your browser settings, or search for a city instead.',
  ...
  error: 'Found your coordinates, but could not resolve a place name.',
};

let instanceCounter = 0;

export const LocationSearch: React.FC<{ onNavigated?: () => void; instanceId?: string }> = ({ onNavigated, instanceId }) => {
  const { location, recentLocations, setLocation, useMyLocation, geoState } = useAppLocation();
  ...
  useEffect(() => {
    ...
    const message = GEO_ERROR_MESSAGES[geoState];
  ...
  return (
    <div ref={containerRef} className="relative w-full max-w-md">
      ...
      <div className="flex items-center gap-2 rounded-xl border border-[var(--color-panel-border)] bg-[var(--color-bg-secondary)] px-3 py-2 focus-within:border-[var(--color-accent-teal)]/60 transition-colors">
        ...
        {query && !loading && (
          <button aria-label="Clear search" onClick={() => onChange('')} className="text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)]">
            <X size={14} />
          </button>
        )}
        <button
      ...
      {isOpen && (
        ...
        >
          <div className="px-3 py-2 text-sm uppercase tracking-wide text-[var(--color-text-muted)] border-b border-[var(--color-panel-border)] flex items-center justify-between">
            <span>{listLabel}</span>
            {query.trim().length < 2 && recentLocations.length === 0 && <span>Try “Delhi” or “London”</span>}
          </div>
          ...
          {!error && loading && (
            <div className="px-3 py-3 text-sm text-[var(--color-text-muted)]">Searching…</div>
          )}

          {!error && !loading && query.trim().length >= 2 && results.length === 0 && (
            <div className="px-3 py-3 text-sm text-[var(--color-text-muted)]">No matching places found.</div>
          )}
          ...
          {!error && !loading && query.trim().length < 2 && recentLocations.length === 0 && (
            <div className="px-3 py-3 text-sm text-[var(--color-text-muted)]">No recent locations yet.</div>
          )}
        </div>
      )}

      <p className="sr-only" aria-live="polite">
        Current location: {location.displayName}
      </p>
    </div>
  );
};
