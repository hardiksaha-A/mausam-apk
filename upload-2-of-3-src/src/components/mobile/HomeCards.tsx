...
const InsightCard: React.FC<{
  ...
  reason: string; isEstimate?: boolean; withPills: boolean;
}> = ({ icon: Icon, title, headline, detail, tone, reason, isEstimate, withPills }) => {
...
export const InsightSection: React.FC = () => {
  ...
  if (preferences.personas.length === 0) {
    ...
    return <InsightCard icon={GENERAL_ICON} title={t('m.general')} headline={g.headline} detail={g.detail} tone={g.tone} reason={g.reason} withPills />;
  }
  ...
  return (
    <div className="space-y-3">
      {preferences.personas.map((p: Persona, i) => {
        ...
        if (!ins) return null;
        return (
          <InsightCard key={p} icon={PERSONA_ICONS[p]} title={ins.title} headline={ins.headline} detail={ins.detail} tone={ins.tone} reason={ins.reason} isEstimate={ins.isEstimate} withPills={i === 0} />
        );
      })}
    </div>
  );
