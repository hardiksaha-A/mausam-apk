...
function windGauge(idn: string, label: LT, value: number, bands: Band[], best: LT, source: LT, u: GuideContext['units'], max = 80): Gauge {
  return { kind: 'gauge', id: idn, label, value, unit: u === 'imperial' ? 'mph' : 'km/h', conv: u === 'imperial' ? kmh2mph : id, decimals: 0, min: 0, max, bands, best, source };
}
    ...
    case 'health': {
      ...
      items.push(
        p && p.available && p.level
          ? { kind: 'note', id: 'pollen', label: { en: 'Pollen', hi: 'पराग' }, text: { en: `${p.dominant ?? 'Pollen'}: ${p.level} right now (CAMS Europe model).`, hi: `${p.dominant ?? 'पराग'}: अभी ${p.level} (CAMS यूरोप मॉडल)।` } }
      ...
      if (m && m.available) {
        if (m.waveHeight != null) {
          items.push({
            ...
            unit: units === 'imperial' ? 'ft' : 'm', conv: units === 'imperial' ? m2ft : id, decimals: 1, min: 0, max: 3, bands: WAVE_BANDS,
      ...
      return [
        {
          ...
          unit: units === 'imperial' ? 'mi' : 'km', conv: units === 'imperial' ? km2mi : id, decimals: 1, min: 0, max: 15, bands: VIS_BANDS,
