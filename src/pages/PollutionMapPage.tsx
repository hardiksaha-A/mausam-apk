  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <h1 className="text-lg font-bold text-[var(--color-text-primary)]">{tr('Pollution Map', 'प्रदूषण मानचित्र')}</h1>
        <DataStatusBadge state={status} />
      </div>

      <Card className="p-0 overflow-hidden">
        <div className="h-[60vh] min-h-[400px] w-full relative">
          <MapContainer center={center} zoom={11} scrollWheelZoom style={{ height: '100%', width: '100%' }} aria-label={tr('Pollution map', 'प्रदूषण मानचित्र')}>
            <TileLayer
            ...
            <Marker position={center}>
              <Popup>
                <div className="text-sm">
                  ...
                  <p className="text-xs mt-1 opacity-70">{tr('Selected location', 'चुना हुआ स्थान')}</p>
                </div>
              </Popup>
            </Marker>
                ...
                >
                  <Popup>
                    <div className="text-sm space-y-1 min-w-[160px]">
                      <p className="font-semibold">{ph(s.name)}</p>
                      ...
                      <div className="grid grid-cols-2 gap-x-3 gap-y-1 pt-1 text-xs">
                        ...
                        <span>{tr('Temp:', 'तापमान:')} <strong>{s.temp ?? '—'}°</strong></span>
                        <span>{tr('Humidity:', 'नमी:')} <strong>{s.humidity ?? '—'}%</strong></span>
                      </div>
                      <p className="text-xs opacity-60 pt-1">{tr('Updated', 'अपडेट')} {s.lastUpdated}</p>
                    </div>
                  </Popup>
                </CircleMarker>
              );
          ...
          <div className="absolute bottom-3 left-3 z-[1000] rounded-lg border border-[var(--color-panel-border)] bg-[var(--color-panel)]/95 backdrop-blur px-3 py-2 shadow-lg">
            <button type="button" onClick={() => setLegendOpen((v) => !v)} aria-expanded={legendOpen} className="text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wide sm:mb-1.5 h-8 sm:h-auto">{tr('Severity Legend','गंभीरता की सूची')} <span className="sm:hidden">{legendOpen ? '▾' : '▸'}</span></button>
            <div className={`${legendOpen ? 'grid' : 'hidden sm:grid'} grid-cols-2 gap-x-3 gap-y-1`}>
              {LEGEND.map((l) => (
                <div key={ph(l.label)} className="flex items-center gap-1.5">
                  <span className="size-2 rounded-full" style={{ backgroundColor: l.color }} />
                  <span className="text-sm text-[var(--color-text-secondary)]">{ph(l.label)}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <Card>
        <CardHeader title={t('pages.pollutionMap.monitoringPoints')} subtitle={preferences.dataMode === 'demo' ? tr('Demo sample stations', 'डेमो नमूना स्टेशन') : tr('Live modeled grid points around your location', 'आपके स्थान के आस-पास के लाइव मॉडल ग्रिड पॉइंट')} />
        <div className="flex items-start gap-2 text-sm text-[var(--color-text-muted)] mb-3">
          <Info size={13} className="mt-0.5 shrink-0" />
          {preferences.dataMode === 'demo' ? (
            <span>{tr('These are clearly-labeled sample stations for demonstration — not real sensors.', 'ये प्रदर्शन के लिए साफ़-साफ़ लेबल किए नमूना स्टेशन हैं — असली सेंसर नहीं।')}</span>
          ) : (
            <span>
              {tr('These points use live atmospheric model data (Open-Meteo / CAMS) at nearby coordinates to visualize spatial variation. They represent modeled grid values, not physical ground-sensor stations.', 'ये पॉइंट आस-पास के निर्देशांकों पर लाइव वायुमंडलीय मॉडल डेटा (Open-Meteo / CAMS) से स्थानिक अंतर दिखाते हैं। ये मॉडल के ग्रिड मान हैं, ज़मीन पर लगे असली सेंसर स्टेशन नहीं।')}
            </span>
          )}
        </div>
        {stations.length === 0 ? (
          status === 'LIVE' || status === 'LOADING' ? (
            <div className="flex items-center justify-center gap-2 py-8 text-sm text-[var(--color-text-muted)]">
              <Loader2 size={15} className="animate-spin" /> {tr('Loading nearby grid points…', 'आस-पास के ग्रिड पॉइंट लोड हो रहे हैं…')}
            </div>
          ) : (
            <EmptyState
              title={t('pages.pollutionMap.noPoints')}
              message={tr('We couldn\'t load nearby grid readings for this location. This is a supplementary layer — the main weather and AQI data above are unaffected.', 'इस स्थान के आस-पास की ग्रिड रीडिंग लोड नहीं हो सकीं। यह एक अतिरिक्त परत है — ऊपर का मुख्य मौसम और AQI डेटा प्रभावित नहीं है।')}
            />
          )
        ) : (
              ...
              return (
                <div key={s.id} className="rounded-xl border border-[var(--color-panel-border)] p-3">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-xs font-semibold text-[var(--color-text-primary)]">{ph(s.name)}</span>
                  ...
                  <p className="text-sm text-[var(--color-text-muted)]">{aqiCat(category)} · {tr('Updated', 'अपडेट')} {s.lastUpdated}</p>
                </div>
              );
            })}
          </div>
