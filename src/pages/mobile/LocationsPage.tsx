...
const LocationsPage: React.FC = () => {
  ...
  const { location, savedLocations, recentLocations, setLocation, saveLocation, removeSavedLocation, isLocationSaved, useMyLocation: locateMe, geoState } = useAppLocation();
      ...
      <GlassCard>
        ...
        {(geoState === 'denied' || geoState === 'unavailable' || geoState === 'timeout' || geoState === 'error' || geoState === 'unsupported') && (
          <p className="mt-2 text-sm text-[var(--color-warning)] leading-snug">
            {geoState === 'denied' ? tr('Location permission is off. Enable it in Android Settings → Apps → Mausam → Permissions, or search for a place above.', 'लोकेशन की अनुमति बंद है। Android सेटिंग्स → ऐप्स → Mausam → अनुमतियाँ में चालू करें, या ऊपर किसी स्थान को खोजें।') : tr('Could not get your location right now. Try again, or search for a place above.', 'अभी आपका स्थान नहीं मिल सका। फिर कोशिश करें, या ऊपर किसी स्थान को खोजें।')}
          </p>
        )}
      </GlassCard>
