...
import { cond, compass } from '../i18n/dynamic';
...
const WeatherPage: React.FC = () => {
  ...
  if (status === 'ERROR' && !weather) {
    return (
      <Card>
        <ErrorState message={errorMessage ?? tr('Weather data is currently unavailable.', 'मौसम का डेटा अभी उपलब्ध नहीं है।')} onRetry={refresh} />
      </Card>
    );
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-bold text-[var(--color-text-primary)]">{tr('Weather Forecast', 'मौसम पूर्वानुमान')}</h1>
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-base text-[var(--color-text-secondary)] mt-1">{cond(weather.current.condition)}</p>
              <p className="text-sm text-[var(--color-text-muted)]">{tr('Feels like', 'महसूस होता है')} {formatTemp(weather.current.feelsLike, preferences.units)}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 flex-1 [&>*:last-child:nth-child(odd)]:col-span-2 sm:[&>*:last-child:nth-child(odd)]:col-span-1">
            <StatCard label={tr('Humidity', 'नमी')} value={weather.current.humidity} unit="%" icon={Droplets} colorVar="--color-accent-blue" />
            <StatCard label={tr('UV Index', 'यूवी सूचकांक')} value={weather.current.uvIndex} decimals={1} icon={Sun} colorVar="--color-warning" />
            <StatCard label={tr('Rain Chance', 'बारिश की संभावना')} value={weather.current.precipitationProbability} unit="%" icon={CloudRain} colorVar="--color-info" />
          </div>
        </div>
      </Card>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard
          ...
          hint={compass(weather.current.windDirectionLabel)}
        />
        <StatCard label={tr('Pressure', 'दबाव')} value={weather.current.pressure} unit="hPa" icon={Gauge} colorVar="--color-accent" />
        <StatCard label={tr('Visibility', 'दृश्यता')} value={weather.current.visibility} decimals={1} unit="km" icon={Eye} colorVar="--color-advanced" />
        <StatCard label={tr('Feels Like', 'महसूस होता है')} value={weather.current.feelsLike} unit="°" icon={ThermometerSun} colorVar="--color-critical" />
      </div>
          ...
          {weather.daily.map((d, i) => (
            <div key={d.date} className="flex items-center gap-4 py-3 first:pt-0 last:pb-0">
              <span className="w-16 text-sm font-medium text-[var(--color-text-primary)] shrink-0">
                {i === 0 ? tr('Today', 'आज') : new Date(`${d.date}T12:00:00`).toLocaleDateString(timeLocale() ?? [], { weekday: 'short' })}
              </span>
              <WeatherIcon code={d.weatherCode} size={20} className="text-[var(--color-accent-teal)] shrink-0" />
              <span className="text-sm text-[var(--color-text-muted)] flex-1 min-w-0 truncate hidden sm:inline">{cond(d.condition)}</span>
      ...
      <div className="grid sm:grid-cols-2 gap-5">
        <Card className="flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-[var(--color-text-muted)]">{tr('Sunrise', 'सूर्योदय')}</p>
        ...
        <Card className="flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-[var(--color-text-muted)]">{tr('Sunset', 'सूर्यास्त')}</p>
      ...
      <p className="text-sm text-[var(--color-text-muted)] text-center">
        {tr('Visibility shown as', 'दृश्यता')} {formatVisibility(weather.current.visibility, preferences.units)} · {tr('Pressure', 'दबाव')} {formatPressure(weather.current.pressure, preferences.units)} · {tr('Wind', 'हवा')} {formatWind(weather.current.windSpeed, preferences.units)}
      </p>
    </div>
  );
};
